...
class LeaverequestsController extends Controller {
	...
	public function index( Request $request ){
		...
		if(is_null($this->user) || !$this->role->hasPermissionTo('LeaveRequests.view')){
            return redirect()->route('dashboard')->with('error', 'You don\'t have enough privileges to perform this action!');
        }

		$tableGrid = \Helper::getTableHeader('Leaverequests');
		$results = Leaverequests::all();
		
		$info = [
			'module_name' => ucwords($this->module),
			'create_button' => 'Leaverequests',
	...
	function create() {
		if(is_null($this->user) || !$this->role->hasPermissionTo('LeaveRequests.create')){
            return redirect()->route('dashboard')->with('error', 'You don\'t have enough privileges to perform this action!');
        }
	...
	function edit( Request $request , $id ){
		...
		if(is_null($this->user) || !$this->role->hasPermissionTo('LeaveRequests.update')){
            return redirect()->route('dashboard')->with('error', 'You don\'t have enough privileges to perform this action!');
        }
	...
	public function destroy($id){
		...
		if(is_null($this->user) || !$this->role->hasPermissionTo('LeaveRequests.delete')){
            return redirect()->route('dashboard')->with('error', 'You don\'t have enough privileges to perform this action!');
        }

		$result = Leaverequests::find($id);
